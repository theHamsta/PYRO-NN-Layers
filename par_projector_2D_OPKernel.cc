 ...
 *
 * Links the parallel-beam projector layer from python to the actual kernel
...
public:
  explicit ParallelProjection2DOp(OpKernelConstruction *context)
      : OpKernel(context) {
    ...
    OP_REQUIRES_OK(context,
                   context->GetAttr("projection_shape", &projection_shape));
    ...
    OP_REQUIRES_OK(context,
                   context->GetAttr("ray_vectors", &ray_vectors_tensor));
  ...
  void Compute(OpKernelContext *context) override {
    ...
    TensorShape out_shape = TensorShape({input_tensor.shape().dim_size(0),
                                         projection_shape.dim_size(0),
                                         projection_shape.dim_size(1)});
