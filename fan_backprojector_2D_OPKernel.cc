 ...
 *
 * Links the fan-beam back-projector layer from python to the actual kernel
 * implementation. Implemented according to Tensorflow API. PYRO-NN is developed
 * as an Open Source project under the Apache License, Version 2.0.
 */
...
void Fan_Backprojection2D_Kernel_Launcher(
    const float *sinogram_ptr, float *out, const float *ray_vectors,
...
public:
  explicit FanBackprojection2DOp(OpKernelConstruction *context)
      : OpKernel(context) {

    // get volume shape from attributes
    OP_REQUIRES_OK(context,
                   context->GetAttr("sinogram_shape", &projection_shape));
    ...
    OP_REQUIRES_OK(context,
                   context->GetAttr("volume_origin", &volume_origin_tensor));
    ...
    OP_REQUIRES_OK(
        context, context->GetAttr("detector_origin", &detector_origin_tensor));
    ...
    OP_REQUIRES_OK(context,
                   context->GetAttr("volume_spacing", &volume_spacing_tensor));
    ...
    OP_REQUIRES_OK(context, context->GetAttr("detector_spacing",
                                             &detector_spacing_tensor));
    auto detector_spacing_eigen = detector_spacing_tensor.tensor<float, 1>();
    detector_spacing = detector_spacing_eigen(0);

    OP_REQUIRES_OK(context,
                   context->GetAttr("source_2_isocenter_distance", &sid));
    OP_REQUIRES_OK(context,
                   context->GetAttr("source_2_detector_distance", &sdd));

    // get rey vectors from attributes
    Tensor ray_vectors_tensor;
    OP_REQUIRES_OK(
        context, context->GetAttr("central_ray_vectors", &ray_vectors_tensor));
  ...
  void Compute(OpKernelContext *context) override {
    ...
    TensorShape out_shape =
        TensorShape({input_tensor.shape().dim_size(0), volume_shape.dim_size(0),
                     volume_shape.dim_size(1)});
    // Check supported batch size. Batch > 1 is not supported currently.
    OP_REQUIRES(context, input_tensor.shape().dim_size(0) == 1,
                errors::InvalidArgument(
                    "Batch dimension is mandatory ! Batch size > 1 is not "
                    "supported in the current PYRO-NN-layers."));
    ...
    Fan_Backprojection2D_Kernel_Launcher(
        input.data(), output.data(), ray_vectors_.data(), number_of_projections,
        volume_width, volume_height, volume_spacing_x, volume_spacing_y,
        volume_origin_x, volume_origin_y, detector_size, detector_spacing,
        detector_origin, sid, sdd);
  }
};

REGISTER_KERNEL_BUILDER(Name(CUDA_OPERATOR_KERNEL).Device(DEVICE_GPU),
                        FanBackprojection2DOp);
