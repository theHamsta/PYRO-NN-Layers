...
__global__ void backproject_2Dfan_beam_kernel(
    float *pVolume, const float2 *d_rays, const int number_of_projections,
  ...
  // Preparations:
  const float2 pixel_coordinate = index_to_physical(
      make_float2(volume_x, volume_y), volume_origin, volume_spacing);
  ...
  for (int n = 0; n < number_of_projections; n++) {
    ...
    float2 intersection =
        intersectLines2D(pixel_coordinate, source_position, central_point,
                         central_point + detector_vec);
    float distance_weight =
        1.0f / (float)length(pixel_coordinate - source_position);
  ...
  pVolume[volume_linearized_idx] =
      sid * sdd * pi * pixel_value / number_of_projections;
...
void Fan_Backprojection2D_Kernel_Launcher(
  ...
  cudaMallocArray(&sinogram_array, &channelDesc, detector_size,
                  number_of_projections);
  cudaMemcpyToArray(sinogram_array, 0, 0, sinogram_ptr,
                    detector_size * number_of_projections * sizeof(float),
  ...
  const dim3 num_blocks = dim3(volume_width / threads_per_block.x + 1,
                               volume_height / threads_per_block.y + 1);

  backproject_2Dfan_beam_kernel<<<num_blocks, threads_per_block>>>(
